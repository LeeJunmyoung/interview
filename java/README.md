...
<p align="center"><img src='img/java-자바코드수행과정.png'></p>
...
<p align="center"><img src='img/java-runtime-data-area.png'></p>
...
#### Heap 영역의 구조
<p align="center"><img src='img/java-heap-memory-structure.png'></p>
...
> 자바 애너테이션은 클래스 파일에 임베디드되어 컴파일러에 의해 생성된 후 자바 가상머신에 포함되어 작동될 수 있다.  

### Annotation 종류

#### 빌트인(Built-in) Annotation
> 자바 SDK에서 지원하는 어노테이션
* @Override : 현재 메서드가 수퍼클래스에 메서드를 오버라이드한 메서드임을 컴파일러에게 명시한다. 만일 슈퍼 클래스에 해당하는 메소드가 없으면 컴파일러가 에러를 발생시켜준다.  
* @Deprecated : 차후 버전에 지원되지 않을 수 있기 때문에 더 이상 사용하는데 자제해야할 메서드를 나타낸다.  
* @SupressWarning : 경고를 제거하는 어노테이션이다. Object형을 엘리먼트로하는 컬렉션을 사용하면, 컴파일러 경고가 발생하는데 해당 어노테이션을 사용하면 개발자가 의도적으로 Object형 사용임을 알려 경고를 없앨수 있다.    
* @SafeVarargs : 자바 7부터 추가되었으며, 생성자나 메소드의 가변인자 파라미터가 안전하게 사용된다는 것을 나타내기 위해 사용합니다. (String… values와 같은 형태)  
* @FunctionalInterface : 자바 8부터 추가되었으며, 인터페이스를 함수형 프로그래밍인 람다 방식으로 사용하려 할때 선언한다.

#### 메타(Meta) Annotation
> 자바 5부터 추가 된 기능으로 커스텀 어노테이션을 만들 수 있다.

* @Retention : 어노테이션이 유지되는 기간을 지정한다. 세가지 유지정책을 사용할 수 있다.   
    1. SOURCE : 소스 파일에만 존재하며, 클래스 파일에는 존재하지 않는다. 
    2. CLASS : 클래스 파일에는 존재하지만 런타임 시에 사용이 불가능. 기본값.
    3. RUNTIME : 클래스 파일에 존재하며 런타임 시 사용 가능. 런타임 시에 리플렉션을 통해 클래스 파일에 저장된 어노테이션 정보를 읽어서 처리할 수 있다.
* @Documented : 어노테이션에 대한 정보가 javadoc으로 작성한 문서에 포함되도록 할때 사용하는 어노테이션. 빌트인 어노테이션 중 @Override와 @SuppressWarnings를 제외하고는 모두 해당 어노테이션이 있다.  
* @Inherited : 어노테이션이 하위 클래스에도 상속되도록 하는 어노테이션.  
* @Target : 어노테이션을 어떤 요소에 적용할지 지정
    1. TYPE : 클래스에 어떤 요소에나 적용가능. 기본값
    2. FUELD : 클래스의 특정 필드
    3. METHOD : 메서드
    4. PARAMETER : 메서드의 파라미터
    5. CONSTRUCTOR : 생성자
